...
把这句话算一遍，比背“assign 再 update”清楚。四个点、两个簇、只走一轮。

数据：

\[
x_1=0,\quad x_2=1,\quad x_3=8,\quad x_4=9.
\]

初始化 centroid：\(c_1=0.5\)（贴近左团）、\(c_2=4\)（还悬在中间）。Assign：每个点选更近的中心。

| 点 | 到 \(c_1=0.5\) | 到 \(c_2=4\) | 赋值 |
|---|---|---|---|
| \(0\) | \(0.5\) | \(4\) | 簇 1 |
| \(1\) | \(0.5\) | \(3\) | 簇 1 |
| \(8\) | \(7.5\) | \(4\) | 簇 2 |
...
Update：簇 1 均值 \((0+1)/2=0.5\)（没动），簇 2 均值 \((8+9)/2=8.5\)。一轮之后中心变成 \(0.5\) 和 \(8.5\)，已经贴住两团。再 assign 一次，赋值不变，算法停。

这一轮做了两件事：离散的 \(z\in\{1,2\}\) 被发明出来；centroid 是簇的平均，不是从某个 \(p(z)\) 抽出来再 decode 的新点。把四个点平均成 \(4.5\) 当“生成”，得到的是两团之间的空白——这正是“无监督 ≠ 会生成”。
...
一个三 mode 玩具。真实数据有三个等权峰：cat、dog、bird。Generator 只学会了 cat，但每张都像照片。抽 9 张展示图，全是猫——fidelity 看起来满分。

按 4.3 的计数：生成的 9 个点全落在 cat 那一块 real manifold 上，precision \(=9/9=1\)。真实侧：cat 被覆盖，dog 和 bird 的真实样本落在 model manifold 外面，recall \(\approx 1/3\)。这就是高 fidelity、低 coverage：mode collapse 的数字脸。

反过来，generator 在三个峰附近都撒点，但大量落在峰与峰之间的“四不像”区：coverage 看起来好，precision 垮。展示时作者会把四不像滤掉，于是你又只看见三张好看的。两维必须分开记，不能让筛选过的网格替你做 recall。
...
概念数字帮这两句落地。两类、两张图。Classifier 对第一张给出 \(p(y\mid x_1)=(0.9,0.1)\)，对第二张给出 \((0.1,0.9)\)。边际 \(p(y)=(0.5,0.5)\)。单张很尖、全体均匀，KL 大约是

\[
0.9\log\frac{0.9}{0.5}+0.1\log\frac{0.1}{0.5}\approx 0.368
\]

两张对称，期望 KL 同为 \(0.368\)，\(IS=\exp(0.368)\approx 1.44\)（两类时理想上限接近 \(2=\exp(\log 2)\)）。若两张都是 \((0.9,0.1)\)，则 \(p(y)=(0.9,0.1)\)，每张 KL 为 0，\(IS=1\)：明确但零多样。若每张都是 \((0.5,0.5)\)，KL 同样是 0。IS 要的是**又尖又散**。

再把“每类一张模板”写进去：100 张图、10 类，每类 10 张像素几乎相同的狗模板。\(p(y\mid x)\) 仍然很尖，\(p(y)\) 仍可均匀，IS 可以很好看——class 内部没有变化，尺子看不见。
...
一维 Gaussian 把两项拆开。真实 \(\mathcal N(0,1)\)，生成 \(\mathcal N(2,1)\)：协方差相同，FID 退化成均值差的平方 \(4\)。若生成改成 \(\mathcal N(0,0.25)\)：中心对齐，方差瘪了，
...
0
+
\bigl(1+0.25-2\sqrt{1\cdot 0.25}\bigr)
=
1.25-1
=
0.25.
...
两个失败数字都能让 FID 变差：一个是整朵云平移（像“全生成成另一类”），一个是云变窄（像 coverage 不够）。标量 \(4\) 和 \(0.25\) 不会自己贴标签。多峰时更糟：三个真实 mode 的均值可能碰巧和只覆盖中间那一个 mode 的生成云接近，均值项几乎不报警。
...
15. 四个点 \(0,1,8,9\)、中心 \(0.5\) 与 \(4\)，一轮 k-means 之后两个 centroid 是什么？这件事为什么仍不是生成？  
16. 三个真实 mode、生成器只打中一个时，precision 和 recall 大致怎样？为什么 9 张全猫的网格测不到 coverage？  
17. 两类上 \(p(y\mid x)\) 分别为 \((0.9,0.1)\) 与 \((0.1,0.9)\) 时，IS 为什么大于“两张都是 \((0.9,0.1)\)”？  
18. 一维 \(\mathcal N(0,1)\) vs \(\mathcal N(2,1)\) 的 FID 是多少？vs \(\mathcal N(0,0.25)\) 又是多少？同一个标量为什么分不清平移和变瘪？
