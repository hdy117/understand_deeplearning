...
**更新里的两个数。** \(W=2.0\)，\(\nabla L_D=0.5\)，\(\alpha=0.1\)，\(\lambda=0.2\)（convention：penalty 是 \(\lambda\|W\|^2\)，梯度带 \(2\lambda W\)）。

无正则：\(W\leftarrow 2.0-0.1\cdot 0.5=1.95\)。

有 L2：

\[
W\leftarrow (1-2\cdot 0.1\cdot 0.2)\cdot 2.0-0.1\cdot 0.5
=(1-0.04)\cdot 2.0-0.05
=1.92-0.05=1.87.
\]

乘法因子 \(0.96\) 先把 \(2.0\) 收成 \(1.92\)，再减数据步 \(0.05\)。同一 \(\alpha\) 下，正则让 \(|W|\) 比纯 GD 更小（\(1.87\) vs \(1.95\)）。若把 penalty 写成 \(\frac{\lambda}{2}\|W\|^2\)，因子变成 \(1-\alpha\lambda\)，同一个 \(\lambda=0.2\) 只会收到 \(0.98\)——所以论文里的 \(\lambda\) 不能跨 convention 横比。

**MAP 怎样长出这个 \(\lambda\)。** 似然若是高斯观测（第 5 章 NLL \(\propto\|y-f\|^2\)），prior

\[
p(W)\propto\exp\bigl(-\|W\|_2^2/(2\sigma_p^2)\bigr)
\]

贡献 \(+\|W\|^2/(2\sigma_p^2)\)。和数据项 \(\frac{1}{2\sigma^2}\|y-f\|^2\) 加在一起，等价于

\[
L_D+\lambda\|W\|^2,
\qquad
\lambda=\frac{\sigma^2}{2\sigma_p^2}.
\]

\(\sigma_p=1\)、数据噪声 \(\sigma=1\) 时 \(\lambda=1/2\)。Prior 再尖一倍（\(\sigma_p=0.5\)）则 \(\lambda=2\)：同一套 GD 乘法收缩更狠。L2 不是另一门课，是 **Gaussian prior 的 \(-\log\)**。
...
**附加项的数量级。** 某处 \(L=0.10\)，\(\|\nabla L\|=2.0\)。连续流不看 \(\alpha\)。离散 GD 相当于在损失上再加

\[
\frac{\alpha}{4}\cdot 4=\alpha.
\]

\(\alpha=0.01\) 时附加 \(0.01\)，几乎看不见；\(\alpha=0.1\) 时附加 \(0.10\)，和 \(L\) 本身一样大——算法会躲开这座“梯度很陡”的山坡，哪怕那里 \(L\) 也不大。换一个已经很平的点 \(\|\nabla L\|=0.1\)，附加只有 \(\alpha/4\cdot 0.01=0.0025\alpha\)，两种步长几乎无差别。隐式正则惩罚的是**陡**，不是惩罚**高**。\(\alpha=2\) 时展开早就不成立，轨迹会振荡，别把公式外推成“\(\alpha\) 越大越好”。
